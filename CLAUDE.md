...
- **Alertas proactivas sobre eventos de seguridad cross-tenant**: la pestaña "Seguridad" de `/audit-command` (2026-08-05) es monitoreo *pasivo* — hay que entrar a revisarla. Falta notificar (email/Slack/in-app) cuando ocurre un `access_blocked` o `login_rejected`, sobre todo si se repite desde la misma IP/usuario (posible fuerza bruta cross-tenant).
