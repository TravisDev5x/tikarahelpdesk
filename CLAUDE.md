...
| Módulo Inventario (port desde HelpdeskECD2026) | **EN PROGRESO** — fases 1-7.5 (port + dashboard/wallboard) cerradas; auditoría ITAM/CMDB 2026-08-21/22: fase Crítico + fases 2.1-2.3 ITAM + fase 3.1 y 3.2 CMDB + fase 4.1 (integraciones opcionales) + fase 5 (Inteligencia) cerradas — solo falta la sincronización real de datos del resto de fase 4 (dejada de lado a propósito, requiere que alguien conecte Entra ID/Intune primero) | Ver `docs/INVENTORY_ROADMAP.md` |
...
- **Footgun disco `public` para archivos subidos por usuarios**: `store("...", ['disk' => 'public'])` guarda en `storage/app/public`, symlinkeado y servido DIRECTO por el webserver -- cualquiera con la URL descarga el archivo sin sesión, sin pertenecer al tenant, sin pasar por ningún middleware de Laravel (una acción `download()`/`show()` con `Gate::authorize()` al lado no ayuda si el frontend igual enlaza a la URL pública en vez de llamar a esa acción). Mordió 3 veces: fotos de activos (`InvAssetImageController`, fase Crítico de la auditoría de Inventario), y adjuntos de Tickets/Incidents (auditoría de bugs críticos, 2026-08-22). Patrón correcto, ya aplicado en los 3: disco `local` (`storage/app/private`, sin symlink) en el alta + columna `disk` por fila (para que los archivos viejos bajo `public` sigan sirviéndose sin migrarlos físicamente) + una acción autenticada que valida tenant antes de `Storage::disk($row->disk)->response()`/`->download()` + el frontend apuntando a ESA acción, nunca a una URL de `/storage/...`. Cualquier `file()->store(...)` nuevo sobre contenido subido por un usuario final debe usar `local`, nunca `public`, salvo que el archivo sea deliberadamente público (ej. logo de marca).
...
| `docs/INVENTORY_ROADMAP.md` | Bitácora de fases del port de Inventario (HelpdeskECD2026 → Tikara) + roadmap de la auditoría ITAM/CMDB (2026-08-21/22) — Crítico/ITAM/CMDB/fase 4.1/fase 5 cerrados, solo falta la sincronización real de datos del resto de fase 4 |
...
- `INVENTORY_ROADMAP.md`: fases 1-7.5 del port cerradas (2026-08-20/21, incluye panel general ligero + wallboard/modo TV). Auditoría técnica ITAM/CMDB (2026-08-21/22): fase Crítico + fases 2.1-2.3 ITAM (ficha técnica, documentos/bajas estructuradas, garantías/fabricantes/ubicación jerárquica) + fase 3.1 CMDB (relación Activo↔Ticket -- `inv_asset_ticket`, `TicketAssetController`) + fase 3.2 CMDB (relaciones entre activos -- `inv_asset_relationships`, `InvAssetRelationshipController`) + fase 4.1 (integraciones opcionales por tenant -- `/inventory/integrations`, tabla `inv_integrations` cifrada con `encrypted:array`, botón "Probar conexión" real vía `App\Services\Integrations\*ConnectionTester`) cerradas. Fase 5 (Inteligencia, 2026-08-22) también cerrada -- mismo patrón de cómputo en vivo que las alertas de fase 7.1, todo en `InvMonitorAlertsService`: `warrantyExpiring()` ahora une `inv_assets.warranty_expiry` con `inv_warranties.ends_at` con severidad graduada (renovación predictiva), `repeatedTransfers()` gana scoring por 2 ventanas (24h/7d) en vez del corte binario fijo, y `problemAssets()` es nuevo ("3+ tickets en 90 días"). Se dejó deliberadamente de lado la sincronización real de datos del resto de fase 4 (requiere que un admin ya haya conectado Entra ID/Intune desde la pantalla de 4.1). Fuera de la auditoría: la vista de asignación consolidada (2026-08-22, `/inventory/assignments`, roster de "quién tiene qué" por usuario) ya se cerró. Sigue pendiente una decisión reconsiderable sobre historial persistente de imports.
