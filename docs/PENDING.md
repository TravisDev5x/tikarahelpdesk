...
| Feature | Doc | Resumen |
|---|---|---|
| Consolidar dashboards de tickets (fase 2) | — | Auditoría de arquitectura 2026-08-08. Actualizado 2026-08-20: se separaron los dashboards por módulo (Tickets → `/resolbeb`, ya existía; Inventario → `/inventory/monitor`, ya existía; `/home` pasa a ser un resumen ligero para el perfil admin — `HomeSummary.jsx`, KPIs + accesos directos, sin gráficas). El `DashboardAdmin` pesado (filtros + `/api/tickets/analytics` + gráficas) que antes vivía en `/home` para admin **sigue en el código, solo dejó de montarse** — no se fusionó a `/resolbeb`. **Sigue pendiente la decisión de producto original**: ¿ese contenido se fusiona a `/resolbeb` como pestaña, se descarta, o se deja como código muerto indefinidamente? |
...
| Panel de administración de `site_user` fuera del onboarding | — | 2026-08-10 (auditado construyendo Fase 7.7): la única pantalla que gestiona `site_user` (staff ↔ sites) hoy es `/onboarding/teams`, y solo asigna a staff que YA aceptó su invitación (decisión de ese sprint -- `site_user.user_id` es FK NOT NULL, no hay dónde guardar una "asignación pendiente" sin agregar columnas nuevas). Si alguien acepta su invitación **después** de que el admin ya terminó 7.7, no existe ninguna pantalla para asignarle un site -- ni siquiera el propio wizard, que ya no vuelve a mostrarse (`onboarding_completed=true`). No bloquea el cierre de Fase 7; queda como mejora real pendiente: un panel de administración de staff/sites aparte (fuera de onboarding). |
...
| `sites` con `unique(client_id, name)` más amplio de lo ideal | — | Detectado 2026-08-10 construyendo Fase 7.5 (alta de Customers/Sites externos en el onboarding): el constraint de la migración `2026_07_11_000005_scope_sites_name_unique_per_client.php` es por **tenant completo**, no por Customer -- dos Customers externos del mismo tenant no pueden ambos tener una sede llamada, ej., "Matriz" (reventaba con `UniqueConstraintViolationException` crudo antes del fix). Compensado con validación de aplicación en `TenantOnboardingController::storeCustomer()` (mensaje claro + regla `distinct` contra duplicados en el mismo envío). **No se cambió el constraint de BD** -- escalarlo a `(client_id, customer_id, name)` es una decisión de modelo de datos más grande, fuera del alcance de ese sprint. Sigue pendiente como mejora real. |
