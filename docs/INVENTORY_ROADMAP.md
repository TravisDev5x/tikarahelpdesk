...
- **Fase 3 (CMDB)** — `inv_asset_relationships` (activo padre/hijo, ej. laptop+dock+monitor como activos independientes vinculados) y, la pieza más importante marcada por el negocio, **relación Activo↔Ticket** (muchos a muchos — hoy no existe ninguna referencia entre `tickets` e `inv_assets`).
...
- **Fase 5 (Inteligencia)** — renovación predictiva, "activos con demasiados tickets" (depende de Fase 3), scoring de anomalías más allá del umbral fijo actual de traslados repetidos.
