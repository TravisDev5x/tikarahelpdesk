...
import { ArrowLeft, ArrowRightLeft, CheckCircle2, Cpu, PackageMinus, Pencil, Plus, Trash2, Upload, UserMinus, UserPlus, Wrench } from "lucide-react";
...
export default function Show() {
    const { asset, categories, statuses, labels, sites, locations, clientUsers, maintenanceOrigins, maintenanceModalities } = usePage().props;
    ...
    const reload = () => router.reload({ only: ["asset"] });
    ...
    return (
        <>
            ...
            <div className="max-w-4xl space-y-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Button asChild variant="ghost" size="icon">
                            <Link href="/inventory/assets"><ArrowLeft className="h-4 w-4" /></Link>
                        </Button>
                        <div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div>
                            <Label htmlFor="asset-images" className="inline-flex items-center gap-2 cursor-pointer text-sm text-brand-muted hover:underline">
            ...
            {/* Asignar */}
            <Dialog open={openDialog === "checkout"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            {/* Devolver */}
            <Dialog open={openDialog === "checkin"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            {/* Trasladar */}
            <Dialog open={openDialog === "transfer"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            {/* Dar de baja */}
            <Dialog open={openDialog === "retire"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            {/* Agregar componente */}
            <Dialog open={openDialog === "add-component"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            {/* Desarmar (despiece) */}
            <Dialog open={openDialog === "disassemble"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            {/* Registrar mantenimiento */}
            <Dialog open={openDialog === "register-maintenance"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
            ...
            <Dialog open={openDialog === "close-maintenance"} onOpenChange={(open) => !open && close()}>
                <DialogContent>
...
Show.layout = (page) => <AuthenticatedLayout title="Detalle de activo">{page}</AuthenticatedLayout>;
